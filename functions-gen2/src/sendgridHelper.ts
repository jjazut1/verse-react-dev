...
import { validateEmailData, logEmailPayload } from './emailValidator';
...
export async function sendEmail(message: sgMail.MailDataRequired): Promise<boolean> {
  try {
    ...
    if (typeof cleanedMessage.to === 'string') {
      ...
      cleanedMessage.to = cleanedMessage.to.map((recipient: any) => {
        if (typeof recipient === 'string') {
          return cleanEmailAddress(recipient);
        } else if (recipient && typeof recipient === 'object' && recipient.email) {
          return { ...recipient, email: cleanEmailAddress(recipient.email) };
        }
        return recipient;
      });
    }
    ...
    if (typeof cleanedMessage.from === 'string') {
      ...
      if (cleanedMessage.from.email) {
        cleanedMessage.from = { 
          ...cleanedMessage.from, 
          email: cleanEmailAddress(cleanedMessage.from.email) 
        };
      }
    }
    
    // Validate the email data before sending
    const validationResult = validateEmailData(cleanedMessage);
    ...
    const validatedMessage = validationResult.data as sgMail.MailDataRequired;
    ...
    const finalMessage = {
      ...validatedMessage,
      from: typeof validatedMessage.from === 'string' 
        ? validatedMessage.from 
        : validatedMessage.from,
      subject: validatedMessage.subject || 'No subject',
      html: validatedMessage.html || '<p>No content provided</p>'
    };
