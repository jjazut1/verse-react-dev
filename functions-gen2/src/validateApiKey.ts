...
function validateSendGridApiKey(apiKey: string): void {
  ...
  if (parts.length === 3) {
    ...
    console.log(`2. Length: ${parts[1]?.length || 0} characters`);
    console.log(`3. Length: ${parts[2]?.length || 0} characters`);
 ...
 */
function findInvalidChars(text: string, urlSafe: boolean = false): void {
  // URL-safe Base64 includes '-' and '_' characters
  const validChars = urlSafe ? /[A-Za-z0-9\-_=]/ : /[A-Za-z0-9+/=]/;
  const invalidChars: {char: string, position: number, code: number}[] = [];
 ...
 */
function checkForProblematicChars(key: string): void {
  if (key.includes(' ')) {
