...
export const sendAssignmentEmail = functions.firestore
  ...
  .onCreate(async (snapshot, context) => {
    try {
      ...
      const assignmentUrl = `${functions.config().app.url}/assignment/${assignment.linkToken}`;
      ...
      // Email content
      const mailOptions = {
        from: `"Verse Learning" <${functions.config().email.user}>`,
...
// Optional: Function to send reminder emails for approaching deadlines
export const sendReminderEmails = functions.pubsub
  ...
  .onRun(async (context) => {
    try {
      ...
      return { success: true, remindersSent: reminderPromises.length };
    } catch (error) {
